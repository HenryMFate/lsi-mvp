...
type OrgPrompt = { id:number; text:string; priority:'high'|'low'; target_day:string|null }
...
export default function Admin(){
  ...
  async function load(){
    ...
    const { data: o } = await sb.from('org_prompts').select('id,text,priority,target_day').order('target_day', {ascending:true})
    ...
    <div className="container">
      ...
      <div className="card">
        ...
        <table style={{width:'100%', borderCollapse:'collapse'}}>
          <thead><tr><th style={{textAlign:'left'}}>Text</th><th>Priority</th><th>Date</th><th></th></tr></thead>
          <tbody>
            {rows.map(r=> (
              <tr key={r.id}>
