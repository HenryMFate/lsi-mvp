-- v27.8 combined delta: ensure pgcrypto for gen_random_uuid, robust id defaults, per-prompt lead_days, RLS
begin;
create extension if not exists pgcrypto with schema public;

-- actions.id default depending on column type
do $$
declare t text;
begin
...
-- org_prompts.id default depending on column type
do $$
declare t text;
begin
  select data_type into t from information_schema.columns
